...
int hash::linearProbing(int val){
    int newIndex=hashFunction(val);
    int i=0; 
    while(ptrToHashTable[newIndex+i]%sizeHashTable !=0){
        i++;
    }
    return (newIndex+i)%sizeHashTable;
}
void hash::insertElement(arr &obj){
    for (int i=0; i<obj.getNumElements(); i++){
        ...
        if(ptrToHashTable[hashTableIndex]!=0){
           hashTableIndex=linearProbing(obj.getArrElement(i));
        }
        else{
...
void hash::searchElement(arr &obj){
    for(int i=0; i<obj.getNumElements(); i++){
        ...
        bool found=false;
        int j=0;
        while(ptrToHashTable[(hashTableIndex+j)%sizeHashTable]!=0){
            if(ptrToHashTable[(hashTableIndex+j)%sizeHashTable]==obj.getArrElement(i)){
